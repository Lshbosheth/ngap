...
try {
  ...
  graph.tour.forEach((step, i) => {
    if (step.order !== i + 1 || !step.title || !step.description || !Array.isArray(step.nodeIds) || !step.nodeIds.length) issues.push(`Invalid tour step[${i}]`);
  ...
  const stats = {
    ...
    assignedFileNodes: assigned.size,
