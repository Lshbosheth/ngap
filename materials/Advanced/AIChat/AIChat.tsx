...
import { Input, Button, Avatar, Spin, Dropdown, Collapse } from 'antd';
        ...
        try{
            request.post('/csf/call/getAIConversation', {params}, {
                ...
                if (res?.returnCode === '0') {
                    ...
                    let content = '';
                    // if(config.props.AIPAdrss === '1' || config.props.AIPAdrss === '2' || config.props.AIPAdrss === '4'){
                        content = res?.bean?.answer || '抱歉，我无法理解您的问题。';
                    // }
        ...
        setMessages([
            {
                id: 'welcome',
                role: 'assistant',
                content: config.props?.welcomeMessage || '您好！我是AI助手，有什么可以帮助您的吗？',
                timestamp: Date.now(),
            },
        ]);
    };
    ...
    useEffect(() => {  // 快捷语数据变化
        ...
        if(quickReplyData){
            if (typeof quickReplyData === 'string') {
                ...
                const resultData = renderFormula(quickReplyData.value, {});
