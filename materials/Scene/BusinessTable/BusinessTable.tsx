...
import { sortConfig, SorterItemProps, getSorter } from '@materials/utils/sort';
import { filterConfig } from '@materials/utils/filter';
import { hasFontColorStrict } from '@materials/utils/textfilter';
import styles from './index.module.less';
import { TagSettingIcons } from '@materials/utils/columns';
import { useDeepCompareEffect } from 'ahooks';
import { getDictionary } from '../../../src/packages/utils/dictionary';
...
const BusinessTable = ({ id, type, config, elements, onCheckedChange }: ComponentType<IConfig>, ref: ForwardedRef<RefConfig>) => {
    ...
    const expandTreeDataForMerge = useCallback(
        (dataList: any[], removeChildren: boolean = false) => {
            ...
            for (const record of dataList) {
                ...
                result.push(Object.fromEntries(removeChildren ? filterChildren : recordEntries));
    ...
    const getParentEndIndex = useCallback(
        (parentIndex: number, childrenColumnName: string): number => {
            ...
            const parentRecord = mergeData[parentIndex];
            if (!parentRecord || !parentRecord[childrenColumnName] || !Array.isArray(parentRecord[childrenColumnName])) {
                return parentIndex + 1;
            }
            const countChildren = (records: any[]): number => {
                let count = 0;
                for (const record of records) {
                    count++;
                    if (Array.isArray(record[childrenColumnName]) && record[childrenColumnName].length > 0) {
                        count += countChildren(record[childrenColumnName]);
                    }
                }
                return count;
            };
            const childrenCount = countChildren(parentRecord[childrenColumnName]);
    ...
    const getVerticalMergeRowSpan = useCallback(
        (record: any, index: number, dataIndex: string, column: any) => {
            ...
            if (isTree) {
                const hasChildren =
                    record && record[childrenColumnName] && Array.isArray(record[childrenColumnName]) && record[childrenColumnName].length > 0;
                if (hasChildren) {
                    return { rowSpan: 1 };
                }
                ...
                while (prevIndex >= 0 && prevIndex >= pageStart) {
                    ...
                    if (prevRecord[dataIndex] === currentValue) {
                        rowSpan++;
                        prevIndex--;
                    } else {
                        break;
                ...
                while (nextIndex < mergeData.length && nextIndex <= pageEnd) {
                    ...
                    if (nextParentIndex !== currentParentIndex) {
                        break;
                    }
                    if (nextRecord[dataIndex] === currentValue) {
                        rowSpan++;
                        nextIndex++;
                    } else {
                        break;
                ...
                if (rowSpan > 1) {
                    ...
                    while (checkIndex >= pageStart) {
                        ...
                        if (checkParentIndex !== currentParentIndex) {
                            break;
                        }
                        if (checkRecord[dataIndex] === currentValue) {
                            currentPosition++;
                            checkIndex--;
                        } else {
                            break;
    ...
    const handleColumnsVisible = (item: any) => {
        if (item.isHide?.expression) {
            try {
                const expressionResult = renderFormula(item.isHide.expression);
            ...
            columns: columnsNew.map((item, index) => {
                ...
                if (typeof title !== 'string') {
                    ...
                    if (titleType == 'variable') {
                        title = renderFormula(title.value, {}, true);
                    ...
                    ? (() => {
                          const handleHeaderCheck = (checkedColKeys: string[]) => {
                              ...
                              if (headerCheckEvent && checkedColKeys.length > 0) {
                                  ...
                                  handleActionFlow(headerCheckEvent.actions, columnData);
                ...
                return {
                    ...
                    onCell(record: any, index?: number) {
                        ...
                        if (tagResult?.textColors) {
                            ...
                            if (textColor && !item.analysisHtml && !hasFontColor) {
                                ...
                                if (tagSettingIconResult.length > 0) {
                                    ...
                                    const getColumnLabel = (dataIndex: string) => {
                                        ...
                                        if (typeof titleObj === 'string') {
                                            ...
                                            titleDisplayValue =
                                                renderFormula(titleObj.value, variableData || {}, true) || titleObj.value || dataIndex;
                                        }
                                ...
                                const handleCellCheck = (
                                    ...
                                    if (cellCheckEvent) {
                                        ...
                                        if (checkedColKeys.length > 0) {
                                            ...
                                            handleActionFlow(cellCheckEvent.actions, { cellDataList });
                                            setStatisticsKey((prev) => prev + 1);
                                        }
                                    }
                                };
                            ...
                            if (item.type === 'money') {
                                ...
                                if (item.analysisHtml) {
                                    txt = util.analysisHtmlToStr(txt, {
                                        replace: (node) => {
                                            if (node.type === 'text' && node.parent) {
                                                return <>{util.formatDate(node.data, 'YYYY-MM-DD')}</>;
                                            }
                                            return undefined;
                                        },
                                    });
                                ...
                                if (item.analysisHtml) {
                                    txt = util.analysisHtmlToStr(txt, {
                                        replace: (node) => {
                                            if (node.type === 'text' && node.parent) {
                                                return <>{util.formatDate(node.data)}</>;
                                            }
                                            return undefined;
                                        },
                                    });
    ...
    }, [data, loading, selectedRowKeys, headerCheckedColumns, cellCheckedRows, originalData, expandedRowKeys]);
    ...
    const isSizeString = (value: any): boolean => {
        if (!util.isNotEmpty(value)) return false;
        const str = String(value).trim();
        const num = '\\d+(?:\\.\\d+)?';
        return (
    ...
    const calculateSum = (
        ...
        if (!data || !dataIndex) return { total: 0, isSizeData: false };
        let total = 0;
        ...
        for (const record of data) {
            ...
            if (includeChildren && record.children && Array.isArray(record.children)) {
                const childResult = calculateSum(record.children as readonly any[], dataIndex, includeChildren);
    ...
    const flattenColumns = (columns: any[]): any[] => {
        ...
        for (const col of columns) {
            const colIsHide = handleColumnsVisible(col);
            if (!colIsHide) {
    ...
    const containerStyle = isVirtual
        ? ({
              ...
              boxSizing: 'border-box' as const, // 盒模型计算
          } as CSSProperties)
        : config.style;
