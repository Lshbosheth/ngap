import { ComponentType, MessageData } from '@materials/types';
import { useState, useImperativeHandle, forwardRef, useMemo, useEffect, memo, CSSProperties, useRef } from 'react';
import { renderFormula, baseApiConvert } from '../../utils/util';
import useMessageListener from '../../../src/utils/useMessageListener';
 ...
 * @returns 拼接后的完整 URL
 */
const buildUrlWithParams = (baseUrl: string, params: any[]): string => {
    if (!baseUrl) return '';
            ...
            if (param.value) {
                if (typeof param.value === 'string' || typeof param.value === 'number') {
                    ...
                    if (param.value.type === 'static') {
                        ...
                        paramValue = String(renderFormula(formula, {}) || '');
                    }
                }
            }
 ...
 * @returns
 */
const IFrame = ({ id, type, config }: ComponentType<IConfig>, ref: any) => {
    ...
    useEffect(() => {
        ...
        // 拼接参数到 URL
        const fullUrl = buildUrlWithParams(baseUrl || '', iframeParams);
    ...
    useMessageListener((event: MessageEvent) => {
        try {
            ...
            if (matchedEvent?.actions && matchedEvent.actions.length > 0) {
                ...
                handleActionFlow(matchedEvent.actions, datas || {});
            }
        } catch (error) {
    ...
    const [messageEventData, setMessageEventData] = useState<MessageData>();
    const resizeHeight = (event: MessageEvent<MessageData>) => {
        setMessageEventData(event.data);
    };
    useEffect(() => {
        if (messageEventData?.name === 'ngapCardHeightChange' && src === messageEventData?.url && !isNil(messageEventData?.height)) {
            setDefaultHeight(`${messageEventData.height}px`);
        }
    }, [src, messageEventData]);
    useEffect(() => {
        window.addEventListener('message', resizeHeight);
        return () => {
            window.removeEventListener('message', resizeHeight);
        };
    }, []);
    // 裁剪后，重新计算高度
    ...
    const baseStyle = useMemo(() => {
        let adaptiveStyle = {};
        if (config?.props?.adaptiveHeight) {
            adaptiveStyle = { height: defaultHeight };
        }
        return { ...config.style, ...mStyle, ...adaptiveStyle };
    }, [config.style, mStyle, config?.props?.adaptiveHeight, defaultHeight]);
