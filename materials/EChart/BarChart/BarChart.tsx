 ...
 * @returns
 */
const EChart = ({ id, type, config }: ComponentType, ref: ForwardedRef<RefConfig>) => {
    ...
    const dealData = (data: Record<string, any> = {}): Record<string, any>[] => {
        ...
        if (!len) return [];
        return xField.reduce((prev: any[], xKey: string) => {
            return prev.concat(
            ...
            <div data-id={id} data-type={type} style={{ ...config.style, ...mStyle }}>
                <Spin spinning={loading} size="large" wrapperClassName="spin-loading">
                    <Bar
                        {...config.props}
                        xField="xField"
                        color={config.props.seriesField ? config.props.color : config.props.color[0]}
                        data={data}
                    />
                </Spin>
            </div>
