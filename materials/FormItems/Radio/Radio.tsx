...
import { isNotEmpty, renderAsyncFormula } from '@materials/utils/util';
...
const MRadio = ({ id, type, config, onLoad, onChange }: ComponentType<IConfig>, ref: ForwardedRef<RefConfig>) => {
    ...
    const formatLabelName = useCallback((label: any) => {
        ...
        if (typeof label === 'object' && label !== null) {
            ...
            if (label.type === 'variable') {
                return renderAsyncFormula(label.value, {});
            }
        }
        return '';
    }, []);
    ...
    useImperativeHandle(ref, () => {
        return {
            ...
            selectItem: (params: Record<string, any>) => {
    ...
    const afterStyle: CSSProperties = useMemo(() => {
        ...
        const itemWidth = fixedColumn && columnsPerRow ? `calc(${100 / Math.min(Math.max(columnsPerRow, 1), 3)}% - 6px)` : undefined;
        return fixedColumn
            ? {
                  width: itemWidth,
                  margin: '5px 0 0 0',
