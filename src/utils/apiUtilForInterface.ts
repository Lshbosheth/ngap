 ...
 * @returns {Promise}
 */
const getFieldFromAPI = (interfaceId: string, selectText?: string, defaultObj = {}, prodflag?: number, state?: any) => {
    console.log("getFieldFromAPI开始----");
    console.log("defaultObj", defaultObj);
    return new Promise((resolve) => {
        request
            .post('/csf/appInterface/getInterfaceParamsAndCheck', { params: { interfaceId: interfaceId, staffId: crossApiUserInfo.getState().userInfo.staffId } })
            .then((data) => {
                const baseInfo = {
                    index: 0,
                    length: 0,
                        ...
                        getIntParmas(treeArr, defaultObj, baseInfo, (params: { [name: string]: any }) => {
                            ...
                            request
                                .post('/csf/appInterface/execInterfaceCp', {
                                    params: { dataInfo: JSON.stringify(allParams), staffId: crossApiUserInfo.getState().userInfo.staffId },
                                })
                                .then(
                                    result => {
                                        resolve(result);
...
export const execInterface = (interfaceId: string, params: {[key: string]: any},options?:any) => {
    return new Promise((resolve) => {
        ...
        .post('/csf/appInterface/execInterfaceCp', {
            ...
            }), staffId: crossApiUserInfo.getState().userInfo.staffId },
        },options)
        .then(
