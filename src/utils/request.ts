...
import { GUIDED_MOCK_APP_TYPES, GUIDED_MOCK_PROJECTS, GUIDED_MOCK_TAG_TYPES } from '@/mock/guidedProcessMock';
...
const getLocalMockResult = <T = any>(url: string, data: Record<string, any> = {}): T => {
    if (isLocalMockMode('guided')) {
        if (url.includes('/appType/queryAppTypeList')) {
            const source = data?.params?.categoryType === '2' ? GUIDED_MOCK_TAG_TYPES : GUIDED_MOCK_APP_TYPES;
            return { returnCode: '0', beans: source.map((item) => ({ ...item })), bean: {} } as T;
        }
        if (url.includes('/app/querySeatTenantList')) {
            return { returnCode: '0', beans: GUIDED_MOCK_PROJECTS.map((item) => ({ ...item })), bean: {} } as T;
        }
        if (url.includes('/app/saveAppInfo')) {
            return { returnCode: '0', beans: [], bean: { id: 'mock-guided-app', belongVersion: 'mock-v1' } } as T;
        }
    }
    return { returnCode: '0', beans: [], bean: {} } as T;
};
...
const request = {
    ...
    get<T = any>(url: string, params: Record<string, any> = {}, options: AxiosRequestConfig = { showError: true, showLoading: false }): Promise<T> {
        if (isLocalMockMode()) return Promise.resolve(getLocalMockResult<T>(url, params));
    ...
    post<T = any>(url: string, data: Record<string, any> = {}, options: AxiosRequestConfig = { showError: true, showLoading: false }): Promise<T> {
        ...
        //     return loadCache(url, data, options);
        // }
        if (isLocalMockMode()) return Promise.resolve(getLocalMockResult<T>(url, data));
    ...
    put<T = any>(url: string, data: Record<string, any> = {}, options: AxiosRequestConfig = { showError: true, showLoading: false }): Promise<T> {
        if (isLocalMockMode()) return Promise.resolve({ returnCode: '0', beans: [], bean: {} } as T);
