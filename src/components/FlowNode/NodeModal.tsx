...
function NodeModal(_: any, ref: any) {
    const [_className, setClassName] = useState(_.className);
    ...
    const [smCont,setsmCont] = useState(true);
    ...
    return (
        <Drawer
            ...
            extra={
                <>
                    <Space>
                        {!smCont && <Button type="primary" onClick={showClick}>
                            显示说明
                        </Button>}
        ...
        >
            {smCont && <Alert
                ...
                description={
                    <div>
                        ...
                        <p>4. 获取表单数据时，可以直接点开行为配置，点击获取表单值，选择对应表单即可。
                            <span onClick={hideClick} style={{marginLeft:"50px",color:'red',cursor:'pointer'}}>隐藏说明</span>
                        </p>
                    </div>
                }
                type="info"
            ...
            />}
            <FlowNode ref={nodeRef} source={_.source} />
        </Drawer>
    );
}
