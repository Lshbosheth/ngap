...
function FlowNode({ source }: { source?: string }, ref: any) {
    ...
    const onEditAction = (node: NodeType) => {
        if ((node.title === '成功' && node.type === 'success') || (node.title === '失败' && node.type === 'fail')) {
    ...
    return (
        <>
            ...
            <ActionModal ref={actionRef} source={flowSource} />
        </>
    );
}
