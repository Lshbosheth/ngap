...
const Toolbar = memo(({ hoverTarget, copyElement, pastElement, delElement }: any) => {
    ...
    const { selectedElement, elements, nodeNav, isUpdateToolbar, setSelectedElement, moveElements } = pageStore(
        ...
        if (element) {
            // 查找父节点
            if (element.parentId && element.parentId.indexOf('undefined') == -1) {
            ...
            // 查找子节点
            const childElement: any = Object.values(state?.page?.pageData?.elementsMap || {}).find((item: any) => item.parentId === element.id);
            if (childElement) {
...
/**
 * 获取 Popover 元素相对于 #editor 容器的位置
 * 通过 viewport 坐标差值计算，正确响应 react-popper 的 transform 定位
 */
const getPopoverPosition = (target: HTMLElement): { width: number; height: number; top: number; left: number } => {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            if (!target) return;
            // Popover 被 react-popper 使用 transform 定位，offsetTop/offsetLeft 不响应 transform
            const style = selectedElement.type === 'Popover'
                ? getPopoverPosition(target)
