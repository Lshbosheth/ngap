...
const CreateProject = (props: { createRef: any; update?: () => void }, ref: any) => {
    ...
    return (
        <Modal title="初始项目信息" open={visible} confirmLoading={loading} onCancel={handleCancel} width={500} footer={null} destroyOnClose>
            <Form
                layout="vertical"
                form={form}
                labelCol={{ span: 5 }}
                wrapperCol={{ span: 24 }}
                initialValues={{ logo: './ngap/imgs/ngap-logo.png' }}
            >
                ...
                <Form.Item label="图标" name="logo" rules={[{ required: true, message: '请上传项目Logo' }]}>
                    <Image width={100} src="./ngap/imgs/ngap-logo.png" />
                </Form.Item>
                <Form.Item>
