...
export default memo(({nodeList, componentIndex, updateScroll, title = '智能诊断'}: any) => {
    ...
    return <div className={style.templateNav}>
        <div className="templateNav">
            <div className="navTitle">
                ...
                <h1>{title}</h1>
                <p>（共诊断<span className="allNodeNums">{nodeList.length}</span>项，其中<span className="abnormalNodeNums">{nodeList.filter((item: any) => String(item.status) === '2').length}</span>项异常）</p>
            </div>
                ...
                <div className="navNodes" ref={navNodes}>
                    <div className="templateNavBox" ref={templateNavBox}>
                        {nodeList.map((item: any, index: number) => {
                            return <div key={item.nodeId || index} className={"templateNavItem showNavNode " + (item.status == 2 ? "errorStatus" : "")} onClick={() => {updateScroll(index)}}>
