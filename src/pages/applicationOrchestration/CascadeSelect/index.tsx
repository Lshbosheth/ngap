...
const CascadeSelectDialog: React.FC<DialogProps> = ({ onSure, onCancel, appCategory, appTypeId }) => {
    // 定义状态变量
    ...
    const saveTempData = () => {
        if (selectIdList.length < 3) {
            const lastSelectedId = selectIdList[selectIdList.length - 1];
            const parentExists = appTypeList.some((item) => item.pId === lastSelectedId);
            if (!parentExists) {
                onSure({
            ...
            if (appCategory == '1') {
                message.error('生产应用对应应用分类有子节点时请选中子节点！');
            } else if (appCategory == '2') {
                message.error('运营应用对应应用分类有子节点时请选中子节点！');
            } else {
                message.error('对应应用分类有子节点时请选中子节点！');
            }
            return;
        }
        onSure({
    ...
    const initLoad = async () => {
        try {
            const result = appCategory ? appTypeListAll.filter((item: any) => item.appTypeCategory === appCategory) : appTypeListAll;
