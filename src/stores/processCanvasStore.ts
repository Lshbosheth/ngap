 ...
 * 流程画布快照数据
 */
export interface ProcessCanvasSnapshot {
    nodeMap: Record<string | number, any>;
    lineArr: any[];
    branchComponentsData: any;
    componentList: any[];
}
...
export interface PageState {
    ...
    // 流程画布历史记录
    processHistoryStack: HistoryRecord[];
    processRedoStack: HistoryRecord[];
}
export interface PageAction {
    ...
    setProcessSaveData: (data: string) => void;
    setComponentsData: (data: any) => void;
    setTargetNodeId: (data: string) => void;
    setComponentListData: (data: any) => void;
    // 流程画布历史记录相关方法
    pushProcessHistory: (record: Omit<HistoryRecord, 'timestamp'>) => void;
    undoProcess: () => HistoryRecord | null;
    redoProcess: () => HistoryRecord | null;
    clearProcessHistory: () => void;
}
export const processCanvasStore = create<PageState & PageAction>((set) => ({
    id: '',
    config: {
        provId: '',
        serviceTypeId: '',
    ...
    updateConfig: (config: PageConfig, id: string, backComponentPage: (state: string) => void) => {
        set(
            produce((state) => {
                state.id = id;
                state.config = { ...config };
                state.backComponentPage = backComponentPage;
            }),
        );
    },
    // 切换编辑模式
        ...
        set(
            produce((state) => {
                ...
                if (element?.config?.props?.formItem) {
                    if (!childElement.config?.props?.formItem) {
                        if (!childElement.config?.props) childElement.config.props = {};
                        childElement.config.props.formItem = {};
                    }
    ...
    setBranchComponentsData(data: any) {
        set(
            produce((state) => {
                state.page.branchComponentsData = data;
            }),
        );
    },
    setProcessSaveData(data: string) {
        set(
            produce((state) => {
                state.page.processSaveData = data;
            }),
        );
    },
    ...
    // 流程画布历史记录 - 推入历史
    pushProcessHistory: (record: Omit<HistoryRecord, 'timestamp'>) => {
        set(
            produce((state) => {
                ...
                if (state.processHistoryStack.length > 50) {
    ...
    // 流程画布历史记录 - 撤销
    undoProcess: () => {
        let undoneRecord: HistoryRecord | null = null;
        set(
            produce((state) => {
                if (state.processHistoryStack.length > 1) {
                    ...
                    undoneRecord = state.processHistoryStack[state.processHistoryStack.length - 1];
                }
            }),
        );
        return undoneRecord;
    ...
    // 流程画布历史记录 - 重做
    redoProcess: () => {
        let redoneRecord: HistoryRecord | null = null;
        set(
            produce((state) => {
