 ...
 * @returns
 */
export const createId = (name: string = 'ngap', len = 35) => {
    return name + '_' + Number(Math.random().toString().substring(2, 12) + Date.now()).toString(36);
};
/**
 * 数字格式化，支持数字、金额、百分百；
 * @param num
 ...
 * 获取页面变量
 */
export function getPageVariable(name?: string, state?: any) {
    const { pageStore } = state;
    const pageData = pageStore?.getState()?.page?.pageData;
 ...
 * 获取页面接口出参
 */
export function getPageApiData(state?: any) {
    const { pageStore } = state;
    const pageData = pageStore?.getState()?.page?.pageData;
...
export function renderAsyncFormula(formula: string, eventParams: any = {}, state?: any, loopVariable: any = {}) {
    try {
        if (!formula) return '';
        // 通过正则获取表单ID
        ...
        const variableData = getPageVariable('', state);
        ...
        const apiData = getPageApiData(state);
        const context = {
            store: userInfo,
 ...
 * @param eventParams 表达式参数，在事件流执行的过程中，如果调用的是脚本运行，则会传入上一个事件流的返回值
 */
export function renderFormula(formula: string, eventParams: any = {}, state?: any, loopVariable: any = {}) {
    try {
        if (!formula) return '';
        // 通过正则获取表单ID
        const formIds: Array<string> = formula.match(/([A-Za-z]+_\w+)\.[\w\\.]*/g) || [];
 ...
 * @param values 表单数据值
 */
export const dateFormat = (list: Array<ComponentType>, values: Record<string, any>, state?: any) => {
    const { pageStore } = state;
 ...
 * 1. 组件属性配置中的变量参数
 */
export const handleParamVariable = (params: any = {}, data?: any, state?: any) => {
    return Object.keys(params).reduce<any>((prev, cur) => {
        ...
        if (typeof variableObj === 'object') {
            ...
            if (variableObj?.type === 'static') {
                ...
                prev[cur] = renderFormula(variableObj.value, data, state);
            } else {
 ...
 * 3. 事件行为中的发送参数
 */
export const handleArrayVariable = (list: any = [], data: any = {}, state?: any) => {
        ...
        if (next.key) {
            if (typeof next.value === 'string') {
                ...
                if (next.value.type === 'static') {
                    ...
                    // 变量不支持模板字符串语法
                    const result = renderFormula(next.value.value, data, state);
 ...
 * @returns 格式化后的字符串
 */
export const formatDateValue = (value: any, format: string): any => {
...
export const formatFormValues = (elementsMap: Record<string, any>, formData: any): any => {
    ...
    // 判断是否格式化日期
    const needsDateConversion = (value: any, type?: string): boolean => {
        if (isEmpty(value)) return false;
        if (type) return true;
        if (dayjs.isDayjs(value)) return true;
        if (typeof value === 'string' && /^\d{4}-\d{2}-\d{2}/.test(value)) return true;
        if (Array.isArray(value)) {
            return value.some((item) => needsDateConversion(item));
        }
        return false;
    };
...
export function numberToChinese(num: number) {
    if (!Number.isFinite(num) || num < 0) return '无效数字';
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if (n === 0) {
            ...
            if (zeroFlag) res += '零';
            res += digit[n] + unit[pos];
            zeroFlag = false;
        }
    }
    // 处理一十、末尾零
    res = res.replace(/^一十/, '十').replace(/零+$/, '');
    return res || '零';
}
