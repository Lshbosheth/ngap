...
import { forwardRef, useEffect, useImperativeHandle, memo, useState, useRef, ForwardedRef, CSSProperties, useMemo } from 'react';
...
const LineChart = ({ id, type, config }: ComponentType, ref: ForwardedRef<RefConfig>) => {
    ...
    const customTooltip = (title: any, items: any[]) => {
        ...
        const { showPreviousNodeLine, seriesField, xField, showDifference } = config.props;
        // 如果启用了对比虚线功能，过滤掉对比线的提示信息,包含 _comparison 的系列名（对比线）
        const filteredItems = showPreviousNodeLine
            ? // 过滤掉包含 _comparison 的系列名（对比线）
              items.filter((item: any) => !(seriesField && item.name.includes('_comparison')))
            : items;
        ...
        filteredItems.forEach((item: any, index: number) => {
            ...
            if (showDifference) {
                ...
                // 获取单位类型用于格式化显示
                const unitType = currentDataPoint?.unitType || 'number';
                ...
                if (currentIndex > 0) {
                    ...
                    if (prevValue === 0 || !isNotEmpty(prevDataPoint.originalValue)) {
                        // 格式化上个节点值显示（使用原始值）
                        let formattedPrevValue;
                        if (prevDataPoint?.originalValue !== undefined) {
                        ...
                        const formattedPrevValue =
                            prevDataPoint?.originalValue !== undefined ? prevDataPoint.originalValue : formatValueWithUnit(prevValue, unitType);
                            ...
                            config.props.xAxisLabelInterval
                                ? {
                                      ...
                                      label: {
                                          ...
                                          formatter: (text: string, _item: any, index: number) => {
                                              // 获取所有X轴数据的总数
