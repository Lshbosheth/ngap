...
const ColumnSetting = memo((props: IModalProp) => {
    ...
    return (
        <Modal title="描述项设置" open={visible} onOk={handleOk} onCancel={handleCancel} width={800} destroyOnClose>
            <Form form={form} labelCol={{ span: 8 }} wrapperCol={{ span: 14 }}>
                <Row>
                    ...
                    <Col span={12}>
                        <Form.Item shouldUpdate label="字段" name="name" rules={[{ required: true, message: '请选择字段名称' }]}>
                            <Select
                                placeholder="请选择字段"
                                options={props.options}
                                onChange={(option) => {
                                    // 当选中一个选项时，更新对应的 name
                                    props.form.setFieldValue(['columns', 'name'], option?.value);
                                }}
                                // 从表单中读取 name 字段
                                value={props.form.getFieldValue('name')}
                            />
                        </Form.Item>
                        <Form.Item label="字段" name="name" hidden rules={[{ required: true, message: '请输入项字段' }]}>
                            <Input />
                        </Form.Item>
                    </Col>
                </Row>
