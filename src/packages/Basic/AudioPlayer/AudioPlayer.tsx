...
import audioFile from './dream.mp3';
...
const AudioPlayer = ({ id, type, config, onPlay, onPause, onResume, onFinish, onLoad, onStop, onWhileplaying }: ComponentType, ref: any) => {
    ...
    const whileplaying = () => {
        console.log('播放中。。。');
        set_seek_bar();
        if (onWhileplaying && mode === 'preview') {
            onWhileplaying();
        }
    };
    ...
    const onresume = () => {
        // console.log('暂停后重新播放')
        mySetState();
        if (onResume && mode === 'preview') {
            onResume();
        }
    };
    const onfinish = () => {
        mySetState();
        if (onFinish && mode === 'preview') {
            onFinish();
        }
    };
    const onload = () => {
