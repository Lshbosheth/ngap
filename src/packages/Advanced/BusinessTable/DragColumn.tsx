...
const DragColumn = memo(({ index, move, handleOpen, add, remove, Len, form, options, onRemove }: any) => {
    ...
    return (
        <>
            <Space style={{ display: 'flex', marginBottom: 15, padding: '0 10px', opacity }} align="center" ref={preview}>
                <HolderOutlined style={{ cursor: 'grab', lineHeight: '32px' }} ref={ref} />
                <Form.Item wrapperCol={{ span: 22 }} name={[index, 'title']} noStyle>
                    <VariableBind placeholder="列名称" style={{ width: '110px', height: '32px', lineHeight: '32px' }} />
                </Form.Item>
                ...
                <EditOutlined onClick={() => handleOpen(index)} style={{ lineHeight: '32px' }} />
                <PlusOutlined onClick={() => add({ title: key, dataIndex: key, width: 120 }, index + 1)} style={{ lineHeight: '32px' }} />
                <MinusOutlined
                    onClick={() => {
                        if (Len === 1) {
