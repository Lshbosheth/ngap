...
import { useState, useEffect, useImperativeHandle, forwardRef, useMemo, useRef, memo, useCallback, CSSProperties, ForwardedRef, Key } from 'react';
import { Tree, Spin } from 'antd';
import type { TreeDataNode, TreeProps, TreeSelectProps } from 'antd';
...
const MTree = ({ id, type, config, onCheck, onExpand, onLoad, onRightClick, onSelect }: ComponentType, ref: ForwardedRef<RefConfig>) => {
    ...
    const getJsonData = function (data: string) {
        try {
    ...
    const dealData = (apiData: Record<string, any[]>) => {
    ...
    const dealChildren = (childrenData: Record<string, any>[], childrenKey: string) => {
        if (!childrenData) return undefined;
    ...
    const onLoadData: TreeSelectProps['loadData'] = useCallback(
        (node: any) => {
            ...
            return new Promise((resolve) => {
                ...
                handleApi(config.api, currentParams, _state)
                    .then((res) => {
                        if (res?.code !== 0) return;
                        if (isNotEmpty(res?.data)) {
        ...
        [config?.props?.fieldNames?.key, config?.props?.asyncKey, config?.props?.fieldNames?.children, data],
    );
    ...
    const onRightClick1 = useCallback(() => {
        if (mode === 'preview') {
            onRightClick?.();
        }
    }, [mode, onRightClick]);
    ...
    const otherProps = useMemo(() => {
        const { async, asyncKey, ...other } = config.props;
        return other;
    }, [config.props]);
    ...
    return (visible && (
                <Spin spinning={loading} size="large" wrapperClassName={classNames(['spin-loading', styles.Tree])}>
                    <Tree
                        ...
                        onDrop={onDrop}
                        draggable={draggable}
                        titleRender={config.props.showEndIcon ? renderTitle : null}
                    />
                </Spin>
            )
    );
